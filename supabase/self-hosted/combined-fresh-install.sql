-- ============================================================================
-- Essay Expert Pro — COMBINED schema for a brand-new Supabase project
-- v3: supabase/self-hosted/setup.sql was written for the OLD production
-- project and assumes legacy tables (mcq_sessions, essay_writing_sessions,
-- knowledge_chapters, knowledge_items, knowledge_chunks, mcq_questions,
-- economics_diagrams, calibration_essays) that only exist there — never on
-- a fresh database — so it is NOT included wholesale here. Everything else
-- in it duplicated what the 20260820060549 migration already creates.
-- This file is just: the full-rebuild migration, the storage-policy
-- migration right after it, and the two genuinely new pieces from
-- setup.sql (permanent-admin protection + the storage bucket).
-- Run this ONCE, top to bottom, in the SQL Editor of a fresh Supabase project.
-- ============================================================================

-- ── from migrations/20260820060549_6f1cc307-12a4-47ab-aaae-68c7a7d80d74.sql ──────────────────────────────────────────
...
CREATE TYPE public.doc_type AS ENUM (
  'coursebook', 'syllabus', 'mark_scheme', 'past_paper', 'mcq_calibration',
  'examiner_report', 'notes', 'other'
);
...
CREATE TRIGGER custom_diagrams_touch BEFORE UPDATE ON public.custom_diagrams
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
-- ── from migrations/20260820060626_83dfa88e-cbff-4a25-bf76-1aa45caaa9b0.sql ──────────────────────────────────────────
...
USING (bucket_id = 'knowledge' AND public.is_staff(auth.uid()));
-- ── fresh-install extras (trimmed from self-hosted/setup.sql) ───────────────
-- Everything else in the original setup.sql either duplicates what the
-- 20260820060549 rebuild migration above already creates (tables, RLS,
-- indexes, match_document_chunks, handle_new_user), or bridges data from
-- legacy tables (knowledge_chapters, knowledge_items, mcq_questions,
-- mcq_sessions, essay_writing_sessions, economics_diagrams,
-- calibration_essays) that only ever existed on the old production project
-- and are never created on a brand-new database. Only the two genuinely new
-- pieces are kept here: the permanent-admin protection, and the storage
-- bucket the app's knowledge-upload RLS policies (in the migration right
-- above) refer to.
...
-- Storage bucket the knowledge_read/knowledge_insert_staff/etc policies
-- (created in the 20260820060626 migration above) apply to.
